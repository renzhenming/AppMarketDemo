...
public class FragmentStack {
    private ArrayList<ArrayList<BaseFragment>> stackList = new ArrayList<>();
    private ArrayList<BaseFragment> stack;
     ...
     */
    public void putStandard(BaseFragment fragment) {
     ...
     */
    public boolean putSingleTop(BaseFragment fragment) {
        ArrayList<BaseFragment> lastList = stackList.get(stackList.size() - 1);
        if (lastList.isEmpty()) {
            lastList.add(fragment);
            return false;
        } else {
            BaseFragment last = lastList.get(lastList.size() - 1);
     ...
     * @return Whether to contain the current instance
     */
    public boolean putSingleTask(BaseFragment fragment) {
     ...
     */
    public void putSingleInstance(BaseFragment fragment) {
        ArrayList<BaseFragment> frags = new ArrayList<>();
        frags.add(fragment);
        stackList.add(frags);
    }
    ...
    public void onBackPressed() {
        ...
        if (i >= 0) {
            ArrayList<BaseFragment> lastStack = stackList.get(i);
    ...
    protected Fragment[] getLast() {
        ...
        for (int x = stackList.size() - 1; x >= 0; x--) {
            ArrayList<BaseFragment> list = stackList.get(x);
