...
import com.rzm.commonlibrary.general.videoeditor.encodedecoder.InputSurface;
import com.rzm.commonlibrary.general.videoeditor.encodedecoder.OutputSurface;
import com.rzm.commonlibrary.general.videoeditor.filter.basefilter.GPUImageFilter;
import com.rzm.commonlibrary.general.videoeditor.filter.helper.MagicFilterFactory;
import com.rzm.commonlibrary.general.videoeditor.filter.helper.MagicFilterType;
...
public class VideoClipper {
    ...
    OnVideoClipFinishListener listener;
    ...
    public void setOnVideoClipFinishListener(OnVideoClipFinishListener listener) {
    ...
    public void setFilterType(MagicFilterType type) {
        ...
        mFilter = MagicFilterFactory.initFilters(mContext,type);
    }
    ...
    private void initAudioCodec() {
        ...
        MediaFormat inputFormat = mAudioExtractor.getTrackFormat(audioTrackIndex);
        int sampleRate = inputFormat.getInteger(MediaFormat.KEY_SAMPLE_RATE);

        MediaFormat format = MediaFormat.createAudioFormat("audio/mp4a-latm", sampleRate, /*channelCount*/2);//这里一定要注意声道的问题
